...
/**
 * Showcase index for the Model interactor sample. Tapping a row opens its detail screen.
 */
@Composable
internal fun ModelShowcaseListScreen(
    onNavigateToDetail: (ModelShowcase) -> Unit,
) {
    WindowScaffold(
        ...
        topAppBar = {
            BackNavTopAppBar(title = stringResource(R.string.model_tool_name))
        },
        content = {
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                itemsIndexed(ModelShowcase.entries, key = { _, showcase -> showcase.name }) { index, showcase ->
                    ShowcaseListItem(
                        showcase = showcase,
                        onClick = { onNavigateToDetail(showcase) },
                        showDivider = index != ModelShowcase.entries.lastIndex,
                    )
                }
            }
        },
